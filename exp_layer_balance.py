"""
Layer Balance Ablation: RibosomeTiny with different embed/upper splits
Tests: {1+5, 3+3, 4+2} at fixed n_chunks=16, 100K steps each.
We already have 2+4 from olares. Runs sequentially on frank (1070, 8GB).
"""
import torch
...
def train_one(embed_layers, upper_layers, device, tokenizer, V):
    tag = f"e{embed_layers}_u{upper_layers}"
    print(f"\n{'#'*60}")
    print(f"# LAYER BALANCE: embed={embed_layers}, upper={upper_layers} ({tag})")
    print(f"{'#'*60}")
    ...
    log_every = 500
    eval_every = 5000
    out_dir = f"./exp_layer_balance/{tag}"
    os.makedirs(out_dir, exist_ok=True)

    train_ds = StreamingTextDataset(tokenizer, 256, "openwebtext")
    train_loader = DataLoader(train_ds, batch_size=batch_size)
    val_loader = get_wikitext_loader(tokenizer, 256, batch_size, "validation", "wikitext-103-raw-v1")
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if (batch_idx + 1) % grad_accum == 0:
            ...
            if global_step % log_every == 0:
                ...
                print(f"  step={global_step:>7d}  CE={mean_loss:.4f}  ETA={eta:.1f}h")
    ...
    elapsed = time.time() - t0
    print(f"\n  {tag}: best_val={best_val_loss:.4f} PPL={np.exp(best_val_loss):.1f} time={elapsed/3600:.1f}h")
    del model, optimizer
    torch.cuda.empty_cache()
    return {"split": tag, "embed": embed_layers, "upper": upper_layers,
            "val_ce": float(best_val_loss), "val_ppl": float(np.exp(best_val_loss)),
            "time_h": round(elapsed/3600, 2)}
...
def main():
    ...
    splits = [(1, 5), (3, 3), (4, 2)]
    results = []
    for embed, upper in splits:
        r = train_one(embed, upper, device, tokenizer, V)
        results.append(r)
        print(f"\n{'='*60}")
        print(f"PROGRESS: {len(results)}/{len(splits)} done")
        for rr in results:
            print(f"  {rr['split']}: CE={rr['val_ce']:.4f} PPL={rr['val_ppl']:.1f}")
        print(f"{'='*60}")

    with open("./exp_layer_balance/summary.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\nAll done. Saved to exp_layer_balance/summary.json")
