"""
Overnight Experiment B: Extended BigBaseline 12L, 100K steps.
Runs on frank (1070, 8GB). Tests if baseline closes the gap.
"""
...
def main():
    ...
    args = argparse.Namespace(
        ...
        output_dir="./overnight_extended",
    )
    model = BigBaseline(vocab_size=V, hidden_size=512, n_heads=8,
                        n_layers=12, max_seq_len=256).to(device)
    print(f"BigBaseline 12L: {model.count_params():,} params, 100K steps")
    train_model(model, "baseline_12L_100K", tokenizer, device, args)
    print("Done.")
