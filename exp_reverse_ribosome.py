"""
Test: RibosomeTiny 3+3 WITH ReverseRibosome (2 layers).
Hourglass architecture: compress in the middle, reconstruct on the way out.

Architecture:
  embed (3L, token-level) -> ribosome (compress 256->16) ->
  upper (3L, chunk-level) -> reverse ribosome (cross-attn + 2L token-level) ->
  LM head
"""
import torch
...
def main():
    ...
    wt_loader = get_wikitext_loader(tokenizer, 256, 8, "validation",
                                     "wikitext-103-raw-v1")

    # RibosomeTiny 3+3 with ReverseRibosome (2 layers)
    print("\n" + "=" * 60)
    print("RibosomeTiny 3+3 + ReverseRibosome (2L)")
    print("  embed(3L) -> compress(256->16) -> upper(3L) -> reverse(2L)")
    print("=" * 60)
    ...
    args = argparse.Namespace(
        ...
        output_dir="./exp_reverse_ribosome/ribo_3p3_rev2",
    )
    val_ce = train_model(model, "ribo_3p3_rev2", tokenizer, device, args,
                         is_ribosome=True)
    ...
    print(f"\n{'='*60}")
    print(f"RESULTS: RibosomeTiny 3+3 + ReverseRibosome (2L)")
    print(f"{'='*60}")
    print(f"  Params:       {total_p:,}")
    print(f"  wikitext-103: CE={wt_ce:.4f}  PPL={math.exp(wt_ce):.1f}")
    print(f"  LAMBADA:      CE={lam_ce:.4f}  PPL={lam_ppl:.1f}  "
          f"Acc={lam_acc*100:.1f}% ({lam_c}/{lam_t})")
    print(f"\n  Compare to PRIOR (no reverse ribosome):")
    print(f"  Params:       49,080,834")
    print(f"  wikitext-103: CE=0.8533  PPL=2.3")
    print(f"  LAMBADA:      Acc=0.0%")
    print(f"\n  Compare to GPT-2 Small:")
    print(f"  Params:       124,439,808")
    print(f"  wikitext-103: PPL=44.4")
    print(f"  LAMBADA:      Acc=58.1%")
    ...
    with open("exp_reverse_ribosome/results.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved to exp_reverse_ribosome/results.json")
